...
import "./Rec.css";
import productsData from "./Women.json";
...
function Womenn() {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6 text-black">
        ...
        <div className="w-full md:w-1/4 bg-white shadow-md rounded-lg p-4">
          <h2 className="text-2xl  gap-1 items-center flex font-semibold mb-4">
            <IoFilterSharp />
            Filters
          </h2>
          ...
          {/* Category Filter */}
          <div className="mb-4">
            <label className="font-bold mb-1 flex gap-1 items-center">
              <MdOutlineCategory />
              Category
            </label>
            <select
    ...
    {productsData.map((items) => (
      <label key={items.id} className="cursor-pointer">
        ...
        <div className="relative flex items-center">
          <input
            ...
            name="size"
            value={items}
            checked={selectedSize === items}
            onChange={(e) => setSelectedSize(e.target.value)}
            className="w-4 h-4 appearance-none border border-gray-300 rounded-full checked:border-black checked:border-[5px] transition-all cursor-pointer"
          />
        </div>
        <span className={`ml-3 text-sm transition-colors ${
          selectedSize === items ? 'text-black font-medium' : 'text-gray-600 group-hover:text-black'
        }`}>
          {items}
        </span>
      </label>
